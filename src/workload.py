...
ORG_DATE_FORMAT="<"+pp.Combine(pp.Word(pp.nums)+"-"+pp.Word(pp.nums)+"-"+pp.Word(pp.nums)+pp.Optional(" "+pp.Word(pp.alphas)))+">"
...
class Project:
    ...
    def timeline_in_period(self, start_date, end_date):
        ...
        raise Exception(f"Couldn't work out where the task {self} sits relative to the given range ({start_date} to {end_date})")
    ...
    def as_bounded_timeline_bar(self, start=None, end=None):
        ...
        return dict(Task=self.name, Start=start.isoformat(), End=end.isoformat(), FTE=round(self.FTE,2))
...
def generate_gantt(projects, start, end):
    bars=[]
    for p in projects:
        if p.timeline_in_period(start,end) not in [TimelineState.NULL, TimelineState.UNKNOWN]:
            bars.append(p.as_bounded_timeline_bar(start=start, end=end))
            print(bars[-1])
    fig = px.timeline(pandas.DataFrame(bars), x_start="Start", x_end="End", y="Task", color="FTE")
...
def generate_report(data, begin_date=None, end_date=None):
    ...
    while cursor_date <= end_date:
        ...
        for p in projects:
            ...
            if timeline_status == TimelineState.NULL: continue
            fte_total+=p.FTE
...
def test():
    ...
    data=[['ABC', '<2026-03-27 Fri>', '<2026-04-03 Fri>', '2d'], ['AI Automation Apprenticeship', '<2026-03-26 Thu>', '<2026-09-01 Tue>', '20d'], ['Concrete International Offer', '<2026-03-28 Sat>', '<2026-04-01 Wed>', '2d'], ['Workload Planning', '<2026-03-26 Thu>', '<2026-03-27 Fri>', '3h'], ['AI Education Bitesize', '<2026-03-23 Mon>', '<2026-04-08 Wed>', '1d'], ['DCP for CU AI Curated Programmes', '<2026-03-23 Mon>', '<2026-04-08 Wed>', '5d'], ['CU AI Playbook', '<2026-03-27 Fri>', '<2026-05-01 Fri>', '5d'], ['CURA Case Study', '<2026-03-27 Fri>', '<2026-07-01 Wed>', '4d'], ['Registry Case Study', '<2026-03-27 Fri>', '<2026-07-01 Wed>', '4d'], ['CEES Case Study', '<2026-03-27 Fri>', '<2026-07-01 Wed>', '4d'], ['AI Hackathon', '<2026-03-27 Fri>', '<2026-06-30 Tue>', '4d'], ['Material Audit', '<2026-03-27 Fri>', '<2026-03-28 Sat>', '2h'], ['Planning for delivery through MTI', '<2026-05-04 Mon>', '<2026-05-22 Fri>', '3d'], ['', '', '', '']]
    generate_report(data, begin_date=date.today(), end_date=date.today()+timedelta(days=62))
